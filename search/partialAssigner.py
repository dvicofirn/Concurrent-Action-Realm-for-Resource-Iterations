...
class PartialAssigner(SearchEngine):
    ...
    def successors_genetic(self, state, steps=1, max_states=1500, flag_h = True):
        """
        ...
        if flag_h:
            ...
            search_results = self.produce_paths_heuristic(state, steps=steps, maxStates=max_states)
            decomposed_successors = []

            for states_tuples, transitions, costs, ncosts, h in search_results:
        ...
        search_results = self.produce_paths(state, steps=steps, maxStates=max_states)
        decomposed_successors = []

        for states_tuples, transitions, costs, ncosts in search_results:
            init_state, new_state = states_tuples
            current_state = state.__copy__()
            stepwise_results = []
